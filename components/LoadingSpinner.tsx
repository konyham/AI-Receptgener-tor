...
const LoadingSpinner: React.FC<LoadingSpinnerProps> = ({ message = 'Recept generálása...', inline = false }) => {
  ...
  const content = (
    <>
      ...
      <p className="mt-4 text-lg text-gray-700 font-semibold text-center">{message}</p>
      <p className="mt-2 text-sm text-gray-500 text-center animate-fade-in">{tip}</p>
    </>
  );
  ...
  return (
    <div className="fixed inset-0 bg-gray-900 bg-opacity-50 flex items-center justify-center p-4 z-50">
      <div className="bg-white rounded-2xl shadow-xl p-8 flex flex-col items-center justify-center w-full max-w-md">{content}</div>
    </div>
  );
};
