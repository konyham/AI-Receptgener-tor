                        ...
                        {expandedCategories[category] && (
                            <ul className="divide-y divide-gray-100 dark:divide-gray-700">
                                {items.map(item => {
                                    ...
                                    return (
                                       <li key={sortedIndex} className="flex items-center justify-between p-3 bg-white dark:bg-gray-800">
                                            <label className="flex items-center gap-3 cursor-pointer">
                                                <input
                                                    type="checkbox"
                                                    checked={selectedIndices.has(sortedIndex)}
                                                    onChange={() => toggleSelection(sortedIndex)}
                                                    className="h-5 w-5 rounded border-gray-300 text-primary-600 focus:ring-primary-500"
                                                />
                                                <div>
                                                    <span className="text-gray-800 dark:text-gray-200">{item.text}</span>
                                                    <span className="block text-xs text-gray-500 dark:text-gray-400">({item.dateAdded || 'nincs dátum'})</span>
                                                </div>
                                            </label>
                                            <button onClick={() => handleRemoveItem(item)} className="text-red-500 hover:text-red-700 p-2 rounded-full hover:bg-red-50 dark:hover:bg-red-900/50" aria-label={`Törlés: ${item.text}`}>
